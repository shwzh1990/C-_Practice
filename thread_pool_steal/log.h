...
class Log
{
	public: 
         Log();
	       ~Log();
	       void init(LogLevel level, bool enable_asyn = true, std::string_view file_path = "./", std::string_view file_name = "my_debug.log");
...
#define LOG_BASE(level, fmt, ...) \
  ...
  {  Log* log = Log::instance(); \
    log->write_log(level, fmt, ##__VA_ARGS__); \
	}while(0);\
